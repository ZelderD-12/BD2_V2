...
export default function RecepcionPage() {
    ...
    (async () => {
      ...
      try {
        ...
        const list = (data.data || []).map((s: any) => ({
          ...
          servicio: String(s.servicio || s.Nombre_Servicio || `Servicio ${s.id_servicio}`),
        }));
  ...
  const generarTicket = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/tickets/generar`, { method: "POST", headers: getAuthHeaders(), body: JSON.stringify(body) });
      const data = await res.json();
  ...
  return (
    <div className="recepcion-page">
      <div className="recepcion-container">
        ...
        {timerActivo && ticketActual && (
          <div style={{ background: tiempoLlamado > 240 ? "#dc3545" : "#fff3cd", color: tiempoLlamado > 240 ? "white" : "#856404", padding: "10px 20px", borderRadius: 10, marginBottom: 20, textAlign: "center", fontWeight: 700, animation: tiempoLlamado > 240 ? "pulse 0.5s infinite" : "none" }}>
            ...
            <div className="panel-card">
              ...
              <div className="card-body">
                <form onSubmit={generarTicket}>
                  ...
                  <div className="form-group"><label><i className="fas fa-stethoscope"></i> Servicio</label>
                    <select value={idServicio} onChange={e => setIdServicio(e.target.value)} required disabled={cargandoServiciosCola || serviciosCola.length === 0}>
                      {cargandoServiciosCola && <option value="">Cargando...</option>}
                      {!cargandoServiciosCola && serviciosCola.map((s) => <option key={s.id_servicio} value={String(s.id_servicio)}>{s.servicio}</option>)}
                    </select></div>
